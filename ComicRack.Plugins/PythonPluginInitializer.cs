...
{
    private static readonly Regex rxComment = new("#\\s*@(?<name>[A-Za-z][\\w_]*)\\s+(?<value>.*)", RegexOptions.Compiled);

    private static readonly Regex rxFunction = new("def\\s+(?<function>[A-Za-z][\\w_]+)", RegexOptions.Compiled);
