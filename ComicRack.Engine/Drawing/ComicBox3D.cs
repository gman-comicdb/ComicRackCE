...
public class ComicBox3D
{
    ...
    public static Bitmap Create(Bitmap cover, Bitmap coverBack, Size size, float thickness, float distance, float rx, float ry, float rz, ComicBox3DOptions options = ComicBox3DOptions.Default)
    {
        ...
        [
            ...
            new(num, num2, 0f, rectangle.Width - 1, 0f),
            new(num, 0f, 0f, rectangle.Width - 1, rectangle.Height - 1),
            new(0f, 0f, 0f, 0f, rectangle.Height - 1)
        ];
        Vertex[] array2 =
        [
            new(0f, 0f, num3, 0f, clip.Height - 1),
            new(num, 0f, num3, clip.Width - 1, clip.Height - 1),
            new(num, num2, num3, clip.Width - 1, 0f),
            new(0f, num2, num3, 0f, 0f)
        ];
        RectangleF rectangleF = new((float)rectangle.Width * 0.1f, (float)rectangle.Height * 0.05f, (float)rectangle.Width * 0.8f, (float)rectangle.Height * num3);
        Vertex[] array3 =
        [
            new(0f, num2, 0f, rectangleF.Left, rectangleF.Top),
            new(0f, 0f, 0f, rectangleF.Right, rectangleF.Top),
            new(0f, 0f, num3, rectangleF.Right, rectangleF.Bottom),
            new(0f, num2, num3, rectangleF.Left, rectangleF.Bottom)
        ];
        Vertex[] array4 =
        [
            new(0f, 0f, 0f),
        ...
        if (options.HasFlag(ComicBox3DOptions.SimpleShadow))
        {
            ...
            array7 =
            [
                new(0f - num4, num2 + num4, num3, 0f, 0f),
                new(num + num4, num2 + num4, num3, size3.Width - 1, 0f),
                new(num + num4, 0f - num4, num3, size3.Width - 1, size3.Height - 1),
                new(0f - num4, 0f - num4, num3, 0f, size3.Height - 1)
            ];
        ...
        try
        {
            using (BitmapFrameBuffer frameBuffer = new(bitmap))
            {
                using (BitmapFrameBuffer texture = new(shadowMap))
                {
                    using (BitmapFrameBuffer bitmapFrameBuffer = new(cover, rectangle))
                    {
            ...
            return rectangle2.Width == 0 || rectangle2.Height == 0 ? null : bitmap.CreateCopy(rectangle2);
        }
        finally
        {
            bitmap?.Dispose();
