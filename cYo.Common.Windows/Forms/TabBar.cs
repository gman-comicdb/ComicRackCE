...
namespace cYo.Common.Windows.Forms
{
    public class TabBar : ContainerControl
    {
        ...
        private void DrawCustom3DBorder(Graphics gr, Rectangle rect, Color disabled, Color active, TabItemState tabItemState)
        {
            if (tabItemState != TabItemState.Selected)
            {
                // Top and Left (light)
                using (Pen disabledPen = new Pen(disabled))
                {
                    gr.DrawLine(disabledPen, rect.Left, rect.Bottom, rect.Left, rect.Top);           // Left
                    gr.DrawLine(disabledPen, rect.Left, rect.Top, rect.Right - 1, rect.Top);         // Top
                    gr.DrawLine(disabledPen, rect.Right - 1, rect.Top, rect.Right - 1, rect.Bottom); // Right
                }
            }
            else
            {
                // Top and Left (light)
                using (Pen activePen = new Pen(active))
                {
                    gr.DrawLine(activePen, rect.Left, rect.Bottom, rect.Left, rect.Top);           // Left
                    gr.DrawLine(activePen, rect.Left, rect.Top, rect.Right, rect.Top);             // Top
                    gr.DrawLine(activePen, rect.Right - 1, rect.Top, rect.Right - 1, rect.Bottom); // Right
                }
                using (Pen disabledPen = new Pen(disabled))
                {
                    gr.DrawLine(disabledPen, rect.Right - 1, rect.Top - 1, rect.Right - 1, rect.Bottom);
                }
            }
        }
        ...
        {
            if (ColorSchemeExtensions.IsDarkModeEnabled)
            {
                // label background
                gr.FillRectangle((tabItemState == TabItemState.Selected) ? SystemBrushes.ControlLightLight : SystemBrushes.ControlDark, rc);


                Border3DSide darkBorder3DSide = Border3DSide.Left | Border3DSide.Top | Border3DSide.Right;
                Color disabled = SystemColors.GrayText;
                Color active = Color.FromArgb(0, 200, 0);
                DrawCustom3DBorder(gr, rc, disabled, active, tabItemState);
                return;
            }
