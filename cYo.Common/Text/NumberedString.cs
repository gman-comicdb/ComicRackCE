...
{
    private static readonly Regex rxBrackets = new("\\s*\\((?<number>\\d+)\\)", RegexOptions.Compiled);

    private static Regex rxRange = new("(?<from>\\d*)\\s*(?<range>-?)\\s*(?<to>\\d*)", RegexOptions.Compiled);
